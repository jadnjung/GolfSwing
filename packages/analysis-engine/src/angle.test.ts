...
function point(x: number, y: number, confidence = 1): {
  x: number;
  y: number;
  confidence: number;
} {
...
describe("calculateJointAngleDegrees", () => {
  it("computes a right angle", () => {
    const angle = calculateJointAngleDegrees(
      point(0, 1),
      point(0, 0),
      point(1, 0),
    );
  ...
  it("is unaffected by the absolute scale of the points", () => {
    const small = calculateJointAngleDegrees(
      point(0, 1),
      point(0, 0),
      point(1, 0),
    );
    const large = calculateJointAngleDegrees(
      point(0, 1000),
      point(0, 0),
      point(1000, 0),
    );
  ...
  it.each([
    ...
    "rejects when landmark %s confidence is below the minimum",
    (_label, a, b, c) => {
      expect(() => calculateJointAngleDegrees(a, b, c)).toThrow(
        InsufficientConfidenceError,
      );
    },
  );
  ...
  it("respects a custom minimum confidence threshold", () => {
    ...
    expect(() => calculateJointAngleDegrees(a, b, c, 0.9)).toThrow(
      InsufficientConfidenceError,
    );
  });
  ...
  it("rejects a zero-length segment (joint coincident with a reference point)", () => {
    expect(() =>
      calculateJointAngleDegrees(point(0, 0), point(0, 0), point(1, 0)),
