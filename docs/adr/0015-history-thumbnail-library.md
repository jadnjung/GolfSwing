...
- **Build-verified on both platforms**: `pod install` + `xcodebuild` (real iOS Simulator, installed and launched, no crash) and `./gradlew assembleDebug` (real Android emulator, installed and launched, no crash) both succeed with the new native module linked. The Android manifest-merger permission stripping was confirmed directly against the actual merged manifest output — `WRITE_EXTERNAL_STORAGE`/`READ_EXTERNAL_STORAGE` are genuinely absent from the final app. Not yet exercised end-to-end with a real recorded swing on either platform (no swing existed on the fresh emulator/simulator installs used for this check) — the thumbnail-generation codepath itself is still only verified via mocked JS tests plus this compile/boot check, not a real video frame extracted on real hardware.
