...
- **iOS not yet build-verified** — the bridging-header/`project.pbxproj` wiring hasn't been run through `pod install` + `xcodebuild` yet; that's exactly the kind of change that looks right on paper but fails on a real build, per the pattern already hit in Steps 11-12 (ADR 0010).
