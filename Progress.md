...
Running record of completed work on the Golf Swing App. Updated after every successfully completed task — newest entries at the top. See `Project.md`/`Architecture.md`/`Decisions.md`/`Roadmap.md` for the full spec (split from the original `docs/PRD.md`) and `docs/adr/` for architecture decisions.
...
Did the full split rather than just fixing the casing: partitioned the old PRD's 23 numbered sections cleanly across the four files by `CLAUDE.md`'s own stated purpose for each — `Project.md` keeps product purpose/scope/requirements (sections 1–6, 9–10, 14–15, 21–22), `Architecture.md` gets technical/ML architecture, dev environment, DevOps, and testing strategy (7–8, 11–13), `Decisions.md` gets the original PRD-level ADR-001–006 (20) with a pointer to the actively-maintained `docs/adr/0001`–`0015` as the real source of truth, and `Roadmap.md` gets the delivery phases, team requirements, definition of done, risks, and the original proof-of-concept plan (16–19, 23). Section numbers were preserved for traceability, and each file's own intro cross-links the other three plus the relevant `docs/` subfolder.

Fixed `CLAUDE.md`'s filename references to match the real Title Case convention — both the newly-introduced `PROJECT.md`/`ARCHITECTURE.md`/`DECISIONS.md`/`ROADMAP.md` and a second, pre-existing instance of the same bug (`PROGRESS.md`/`CHECKLIST.md` vs. the real `Progress.md`/`Checklist.md`), caught only because `prettier --check` flagged unrelated bullet-style drift in the same uncommitted rewrite and a closer read turned up the casing mismatch too. Updated every live reference to the old `docs/PRD.md` path — `README.md`, `SECURITY.md`, `Checklist.md`, `Progress.md`'s header note, and the four `docs/architecture/`/`docs/qa/` reports that cited specific PRD section numbers — to point at the correct new file per section. Did not touch historical dated entries elsewhere in this log that mention `docs/PRD.md`; those are accurate records of what was true at the time and rewriting them would misrepresent history.

`docs/PRD.md` itself was already physically removed (matching its staged deletion) by the time this was picked up.
...
**Built**: `swingRepository.getSwingThumbnail(swingId)` — generates a thumbnail once via `createThumbnail`, then moves it into the swing's _own_ directory (`thumbnail.jpg` next to `source.mp4`), not the library's separate cache — specifically so `deleteSwing`'s existing `unlink(swingDir)` cleans it up automatically too, actually satisfying a PRD 9.8 requirement that was already sitting in a code comment on `deleteSwing` since it was written ("deleting a swing must remove the original video, thumbnail, pose data, metrics, and feedback together"). `HistoryScreen`'s `SwingRow` now shows this thumbnail via a new `SwingThumbnail` component, falling back to a placeholder icon (not a crash or blank space) while loading or if generation fails.
...
**Critical bug 2 fixed — developer reasoning shown as user-facing copy**: `RecordScreen`'s note under the frame-rate selector read _"Simultaneous front-and-rear recording isn't offered — it requires checking the device's actual concurrent-camera capability, which needs a real device to verify"_ verbatim — that's ADR/commit-message language, not product copy. Rewritten to "Recording both cameras at once isn't available yet." The real reasoning stays in code comments and `docs/qa/device-matrix.md`, where it belongs.
...
**MVP item 5 closed**: wired the frame-rate selector to `useCameraFormat` instead of it being UI-only — the resolved device format's `minFps`/`maxFps` clamp the requested rate, the manifest records the _actual_ fps used (not the blind selection, since `ReplayScreen`'s frame-stepping depends on this being accurate), and a note tells the user when their selection got clamped. This was explicitly blocked on "needs a real device to verify" until today.
...
## 2026-08-04 ~01:45 KST — Git history cleanup, V1→main merge, and two-person collaboration setup

**Git history cleanup:** the user asked whether the old "Gitlab UW" git identity (a leftover from a previous CSE332 course repo, discovered earlier via `git log`) existed anywhere else. Checked global gitconfig (already fixed to "Jadn"/jadenjung1004@gmail.com), SSH key comment, hostname, npm/gem/ssh configs, env vars — all clean. The one real remnant: 21 historical commits on `V1` (all before the identity fix) carried the stale author/committer `Gitlab UW <git@gitlab.cs.washington.edu:...>`. With explicit user confirmation (confirmed no other clones of this repo exist, repo confirmed public but 0 forks/stars/watchers), rewrote those commits' author/committer via `git filter-repo --mailmap` (installed via `brew install git-filter-repo`) and force-pushed the corrected history to `origin/V1` only — `main` was not touched (it never had the bad identity; the "Initial commit" on `main` was pre-`V1`-history, a separate GitHub-generated commit). Verified thoroughly before and after: `git fsck --full --strict` clean, full commit-by-commit tree-hash diff between old and new history showed **zero content differences**, and the full `pnpm -r lint/typecheck/test` suite still passed. One real mistake happened mid-operation (a `git checkout main` silently failed on an uncommitted `CLAUDE.md` conflict, and a subsequent `git reset --hard origin/main` landed on `V1` instead, briefly wiping the rewritten history locally) — caught immediately before anything was pushed, recovered via `git fsck --unreachable` finding the still-intact rewritten commit, and the also-lost stashed `CLAUDE.md` edit was separately recovered from a dangling stash commit found the same way. Both this incident and the final all-clear verification were reported to the user in full.

**V1 → main merge:** the user then merged `V1` into `main` (previously just GitHub's single auto-generated "Initial commit" with a placeholder README). Used `git merge V1 --allow-unrelated-histories` (the two branches shared no common ancestor) — the only conflict was `README.md` (main's placeholder vs. V1's real project README), resolved in favor of V1's version. Verified `main` now has all 188 tracked files with zero diff against `V1`, full validation suite passing. Pushed to `origin/main` — GitHub's response noted this bypassed a "changes must go through a PR" branch-protection rule on `main` (allowed only because the user has bypass/owner privilege) — flagged this to the user since it means that protection rule is real and would block anyone without that privilege.

**CLAUDE.md changes** (all explicitly requested by the user, each individually confirmed before committing/pushing — see the new standing rule below):

- Added a "Protecting secret information" subsection under Safety: documents the app's current local-only/no-secrets-needed posture as deliberate (verified empirically — zero `process.env`/API-key/token references or `.env` files anywhere in the app source), the one existing exception (the RN template's public-credential debug keystore, `apps/mobile/android/app/debug.keystore` — normal to commit, unlike a release keystore), the existing Gitleaks CI scan as the real safety net, and what to do if a secret is ever actually leaked (rotate first, history-removal is a separate explicit decision).
- Added a new standing rule to Repository Scope: **this agent must never edit, commit, or push changes to `CLAUDE.md` itself unless the user explicitly says so in that specific conversation** — not implied by a broader "commit everything" request. Confirmed with the user directly: a generic "commit and push everything" does _not_ cover `CLAUDE.md`; it must be named explicitly, and any other pending changes should still be committed/pushed normally while `CLAUDE.md` is called out as skipped.

**Two-person collaboration setup:** the user is bringing on a second person. Discussed and the user is handling directly: adding them as a GitHub collaborator, updating `.github/CODEOWNERS` (currently just `@jadnjung`) with their handle themselves. Talked through the branching model given `main`'s existing PR-required protection rule: recommended each person work on their own branch (the user proposed `V1` for themselves and a `V2` for the second person, both based off `main`) rather than both committing onto one shared branch — the isolation comes from _each person owning their own branch_, not from which branch is "the shared one." Explained the actual merge mechanics for when both people's branches merge into `main` (git combines both sets of changes via the common ancestor; conflicts only arise on lines both people actually touched, otherwise both features land together automatically) and recommended periodically syncing each personal branch from `main` after merges land, rather than letting them drift indefinitely.

**Nothing code-related changed in this entry** — no lint/typecheck/test re-run was needed beyond what's already reported above (the filter-repo and merge verification runs).
...
**Built:** `ReplayScreen` gained an "Export" button (shown regardless of playback state, since export operates on the file path directly, not the player) that calls `Share.open({ url: 'file://<path>', type: 'video/mp4', filename, failOnCancel: false })`. `failOnCancel: false` makes the user dismissing the share sheet _resolve_ the promise (with `dismissedAction: true`) rather than reject it — checked and treated as a normal, silent outcome; a genuine failure (e.g. no app can handle the file type) still surfaces an alert.
...
**Repository:** `swingRepository.setSwingTags(swingId, tags)` reads and _parses_ the existing manifest (via `parseSwingManifest`, not a raw read) before rewriting it with only `tags` changed — a deliberate choice: it fails loudly on a corrupt manifest rather than silently overwriting one with a partial/malformed one.
...
- The "add a tag" test initially called `onChangeText` and `onSubmitEditing` back-to-back inside one `act()` block — but `onSubmitEditing`'s closure (via `addTag`'s `useCallback`) was captured from the render _before_ `onChangeText`'s state update had flushed, so it still saw the old (empty) draft text. Fixed by splitting them into separate `act()` calls, letting React re-render between them — the same category of stale-closure timing issue, just in a new shape.
- Adding tests that open the `TagEditorModal` (more re-renders of the underlying `FlatList`) tipped a previously-dormant issue into actually failing: `VirtualizedList`'s internal debounced `setState` (`_updateCellsToRenderTimeoutID`) fired _after_ the test file finished, and Jest treats a `console.error` logged post-teardown as a hard failure — invisible in the per-test-suite pass/fail summary, but `pnpm test`'s aggregate exit code was 1 despite every individual test passing. Fixed by hoisting `tree` to `describe`-scope and unmounting it in `afterEach` (the same pattern already used in `App.test.tsx` since Step 10), which lets `VirtualizedList` clear its pending timer via `componentWillUnmount` before the file's context tears down.
...
### Where things actually stand (as of commit `63a88dd` on `V1`, 2026-08-04 ~01:45 KST)

**Branch state**: `V1` is the active working branch (current checkout) and is ahead of `main` — `main` has everything through the merge commit (`1906663`) but is missing `V1`'s commits since then (the two `CLAUDE.md` changes, `ab17f8a`/`63a88dd`). `main` and `V1` are no longer identical; check `git log --oneline main..V1` in a new session to see exactly what hasn't reached `main` yet. Two-person collaboration is being set up: the user works on `V1`, a second person will work on their own branch (proposed `V2`), both merging into `main` via PRs (protected, requires PRs — a real rule, confirmed via a bypass notice on push). See the dated entry directly above this section for full detail on the git-history rewrite, the V1→main merge, and the collaboration setup discussion.
