...
Both native-independent items flagged after Step 9 are now done (Step 10). Remaining native-independent candidates: a Settings screen to edit the profile after onboarding (currently one-time only), or `calculateVectorAngleDegrees` from the swing-angle-definitions doc (though that's arguably premature without real landmark data to test it against, per the same reasoning as ADR 0009). Otherwise, further substantial progress is genuinely blocked on real device access (Phase 2 pose inference) or a human/business decision (remaining Phase 0 items, PRD section 22). Worth checking with the user on priority again rather than guessing which thin remaining slice matters most.
